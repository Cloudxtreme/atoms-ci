#!/bin/bash
# Example paramaeters
# [ -z "${GROUP_ID}" ] && export GROUP_ID=""
# [ -z "${ARTIFACT_ID}" ] && export ARTIFACT_ID=""
# [ -z "${VERSION}" ] && export VERSION="1.1.1-SNAPSHOT"
# [ -z "${TYPE}" ] && export TYPE="war"
# 
# Example Usage
# ./download-artifact.sh -t -a ${GROUP_ID}:${ARTIFACT_ID}:${VERSION} -e ${TYPE} -r libs-snapshot-local -n https://ci.atomsd.org -o /tmp/${ARTIFACT_ID}-${VERSION}.tar.gz
